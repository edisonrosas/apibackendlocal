...
exports.deletePost = (req, res) => {
  Post.findOneAndDelete({ _id: req.body.postId, author: req.userData.userId })
    .then((post) => {
      if (!post) return res.status(401).json({ message: "Error al borrar" });
     ...
     // deletePostPhoto(post);
      ...
      PostLike.findOneAndDelete({
        post: mongoose.Types.ObjectId(post._id),
      }).then(() => console.log("Likes borrados"));
      Notification.deleteMany({
        post: mongoose.Types.ObjectId(post._id),
      }).then(() => console.log("Notificaciones borradas"));

      res.status(200).json({ message: "Publicación Borrada", id: post._id });
    })
